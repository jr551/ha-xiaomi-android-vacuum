...
Litter Tray Vacuum Cleanup is a Home Assistant custom integration for one safe,
repeatable cat-litter cleanup workflow. It reacts to a genuine MiniNook litter counter increase,
...
remains in `hermes/` because the cleanup scheduler still uses it for notifications and opt-outs.
...
- The scheduler persists the job before sending a family notification.
- Only ⏭️, ❌, or 🛑 on the exact message can skip the exact job.
- Immediately before motion, the scheduler refreshes the direct map, checks the bridge
...
restart Home Assistant, and add **Litter Tray Vacuum Cleanup** from
...
The legacy `sui_hooverbot` domain is intentionally retained for compatibility;
the rebrand does not rename existing entities, services, stored jobs, or the
registered webhook. Home Assistant branding assets live under
`custom_components/sui_hooverbot/brand/`.
...
It is intentionally independent of this cleanup scheduler and contains no vacuum coordinates or
